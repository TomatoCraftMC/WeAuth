...
class AdminCLI:
    ...
    def admin_cli(command: str, game_server: MCServerConnection) -> (int, str):
        ...
        if command_list[0] == "op":
            ...
            mine_id, mine_num, cdkey_num, comment = (
                command_list[1],
                command_list[2],
                command_list[3],
                command_list[4],
            ...
            try:
                gift_hash = CDKey.create_gift(
                    gift_arg=mine_id,
                    gift_num=int(mine_num),
                    gift_total=int(cdkey_num),
                    gift_comment=comment,
                )
                cdkey_list = CDKey.generate_cdkey(
                    gift_hash=gift_hash, gift_total=int(cdkey_num), is_feedback=True
                )
            except Exception:
            ...
            if len(command_list) != 2:
                return 0, "参数错误，正确用法:\n!d [player_id]"
            msg = AdminCLI.remove_by_player_id(
                game_server=game_server, play_id_from_wechat=command_list[1]
            )
            if msg is None:
            ...
            DB.update_item_by_player_id(
                player_id=player_id, ban=command_list[2], sub=command_list[3]
            )
            return_code = -200
            if command_list[2] == "0":
                return_code, msg = DB.push_to_server_ban(
                    player_id=player_id, game_server=game_server, mode=0
                )
            elif command_list[2] == "1":
                return_code, msg = DB.push_to_server_ban(
                    player_id=player_id, game_server=game_server, mode=1
                )
            if return_code != 200:
                return 0, f"{player_id} 的封禁与注册信息已更新，但推送游戏服务器失败。"
            return (
                0,
                f"{player_id} 的封禁与注册信息已更新！\n您可以使用!s {player_id}进行查看。",
            )
    ...
    @staticmethod
    def remove_by_player_id(
            game_server: MCServerConnection = None, play_id_from_wechat=None
    ) -> Optional[str]:
        if play_id_from_wechat is None:
            ...
            return_code, msg = DB.push_to_server_whitelist(
                player_id=player_id, game_server=game_server, mode=0
            )
            if return_code != 200:
                print(
                    f"-玩家 {player_id} 在数据库成功删除，但推送至游戏服务器时出现异常"
                )
                return (
                    f"-玩家 {player_id} 在数据库成功删除，但推送至游戏服务器时出现异常"
                )
    ...
    def ban_db(player_id: str, game_server: MCServerConnection = None) -> Optional[str]:
        if game_server is not None:
            ...
            return_code1, msg1 = DB.push_to_server_ban(
                player_id=player_id_, game_server=game_server
            )
            return_code2, msg2 = DB.push_to_server_whitelist(
                player_id=player_id_, game_server=game_server, mode=0
            )
    ...
    @staticmethod
    def unban_db(
            player_id: str, game_server: MCServerConnection = None
    ) -> Optional[str]:
        if game_server is not None:
            player_id_ = DB.search_player_id(player_id=player_id)
            if player_id_ is None:
            ...
            if return_code != 200:
                return msg_encode(
                    f"数据库中已解除封禁{player_id_}，但推送游戏服务器失败"
                )
