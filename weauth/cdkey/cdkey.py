...
class CDKey:
    ...
    def cdkey_cli(cdkey: str, player_id: str, game_server: MCServerConnection) -> str:
        ...
        try:
            ...
            gift_arg, gift_num, gift_valid = CDKey.check_gift_arg_and_num(
                gift_hash=gift_hash
            )
            if not gift_valid:
                return "该CDKey已停用"
        ...
        print("here1")
        return_code, msg = game_server.push_command(
            command=f"give {player_id} {gift_arg} {gift_num}"
        )
        if return_code != 200:
            return "礼物发送失败"
            ...
            gift_arg, gift_num, gift_valid = CDKey.check_gift_arg_and_num(
                gift_hash=gift_hash, is_delete=True
            )
            return "成功兑换！礼物已成功发送"
        elif (
                game_server.server_type.upper() == "RCON" and msg[:7] == "Unknown"
        ):  # gift_arg 不合法
            ...
            if cdkey in result[key]:  # 查找与删除应该分离
                if is_delete:
                    ...
                    with open("cdkey.yaml", "w+") as f:
                        yaml.dump(
                            data=result, stream=f, allow_unicode=True, sort_keys=False
                        )
                return key
    ...
    def create_gift_entrypoint() -> None:
        gift_comment: str = input(
            "-请输入礼物注释,并按回车确认。例如: 火把/钻石/给小张的礼物\n> "
        )
        gift_num: int = int(
            input("-请输入单次兑换所给予的数量,并按回车确认。例如: 6\n> ")
        )
        gift_arg: str = input(
            "-请输入礼物,即Minecraft的物品ID,可以带有NBT标签。例如：\n"
        ...
        try:
            ...
            CDKey.generate_cdkey(gift_hash=gift_hash, gift_total=gift_total)
    ...
    @staticmethod
    def create_gift(
            gift_arg: str, gift_num: int, gift_total: int, gift_comment="无礼物注释"
    ) -> str:
        ...
        context = {
            gift_hash: {
                "gift_arg": gift_arg,  # 礼物 如 minecraft:command_block  可以带NBT标签
        ...
        try:
            ...
            parts = [
                "".join(
                    ...
                    for _ in range(CDKEY_LENGTH_ONE_PIECE)
                )
                for _ in range(4)
            ]
            return "-".join(parts)
            ...
            raise ValueError(
                "CDKEY_LENGTH_ONE_PIECE must be defined and should be a positive integer"
            )

    @staticmethod
    def generate_cdkey(
            gift_hash: str, gift_total: int, is_feedback=False
    ) -> Optional[list]:
        cdkey_list: list[str] = []
