...
def main(args) -> None:
    ...
    print(
        "\033[34mWeAuth is released under the GNU GENERAL PUBLIC LICENSE v3 (GPLv3.0) license.\033[0m"
    )
    ...
    # 测试微信服务器连接
    access_token = test_wechat_server(
        app_id=config["appID"], app_secret=config["AppSecret"]
    )

    # 公众号回复语句
    responses = {"welcome": config["welcome"]}  # 玩家注册白名单成功

    # 创建监听进程
    weauth_server = Process(
        target=server_start, args=(config, responses, url, game_server)
    )
...
def server_start(
        config: dict, responses: dict, url: str, game_server: MCServerConnection
) -> None:
    # 创建Flask实例
    print("-正在启动监听......\n")
    listener = Listener(
        wx_user_name=config["WxUserName"],
        responses=responses,
        url=url,
    ...
    if config["ssl"] == 1:
        ...
        try:
            ssl_context.load_cert_chain(
                certfile=config["ssl_cer"], keyfile=config["ssl_key"]
            )
        except FileNotFoundError:
            print("-未找到ssl证书文件！")
            sys.exit(0)
        server = pywsgi.WSGIServer(
            ("0.0.0.0", 443), listener.wx_service, ssl_context=ssl_context
        )
...
def backup_server(cron: str) -> None:
    try:
        ...
        print(
            "\033[31m-备份系统计划设置错误，请检查config.yaml中的backup_cron设置是否正确\033[0m"
        )
        sys.exit(0)
