...
def entrypoint():
    ...
    parser.add_argument(
        "-list",
        "--list",
        help="打印数据库里面的所有Player_id",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-search", "--search", help="数据库中搜索Player_id", type=str, default="-1"
    )
    parser.add_argument(
        "-update",
        "--update",
        help="更新数据库中一条,传入玩家ID",
        type=str,
        default="-1",
    )
    ...
    parser.add_argument(
        "-s", "--s", help="切换注册状态", default=False, action="store_true"
    )
    parser.add_argument(
        "-unban", "--unban", help="unban [player_id]", default="-1", type=str
    )
    ...
    if args.wechat_confirm:
        ...
        if args.pub_path == "-1" or args.key_path == "-1":
            ...
            confirm(
                args.token, url=args.url, cer_path=args.pub_path, key_path=args.key_path
            )
            sys.exit(0)
...
def update_item(args: argparse.Namespace) -> None:
    ...
    if _ban and bool(ban_):
        ans1.append(
            input(
                f"-您确认将玩家 {player_item[0]}的封禁状态更改为【否】？ (输入yes/no确认)\n>"
            )
        )
        ans1.append("0")
        ...
        ans1.append(
            input(
                f"-您确认将玩家 {player_item[0]}的封禁状态更改为【是】？ (输入yes/no确认)\n>"
            )
        )
        ans1.append("1")
    else:
    ...
    if _sub and bool(sub_):
        ans2.append(
            input(
                f"-您确认将玩家 {player_item[0]}的注册状态更改为【否】？ (输入yes/no确认)\n>"
            )
        )
        ans2.append("0")
        ...
        ans2.append(
            input(
                f"-您确认将玩家 {player_item[0]}的注册状态更改为【是】？ (输入yes/no确认)\n>"
            )
        )
        ans2.append("1")
    else:
    ...
    DB.update_item_by_player_id(player_id=player_item[0], ban=ans1[1], sub=ans2[1])
    print("-数据库成功更新")
    sys.exit(0)
