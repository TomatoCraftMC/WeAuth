...
class DB:
    ...
    def add(player_id: str, openid: str, game_server: MCServerConnection) -> (int, str):
                ...
                if user[2] == 1:
                    ...
                    if user[3] == 0:  # 注册取关后重新注册
                        return_code, msg = DB.push_to_server_whitelist(
                            player_id=player_id, game_server=game_server
                        )
                        DB.responses_check_whitelist(return_code, msg)
                        cur.execute(
                            "UPDATE players SET ISSUB=? WHERE OPENID=?", (1, openid)
                        )
                        conn.commit()
                        cur.close()
                        conn.close()
        ...
        try:
            ...
            DB.responses_check_whitelist(return_code, msg)
            cur.execute(
                "INSERT INTO players values(?,?,?,?,?)", (player_id, openid, 0, 1, 0)
            )
            conn.commit()  # 全新注册
            cur.close()
            conn.close()
    ...
    def update_item_by_player_id(player_id: str, ban: str, sub: str) -> None:
        ...
        cur = conn.cursor()
        cur.execute(
            "UPDATE players SET ISBAN=?, ISSUB=? WHERE ID=?",
            (int(ban), int(sub), player_id),
        )
        conn.commit()
        cur.close()
        conn.close()
    ...
    def ban_player_id(player_id: str, mode=1) -> None:
        ...
        if mode == 1:
            cur.execute(
                "UPDATE players SET ISBAN=?, ISSUB=? WHERE ID=?", (1, 0, player_id)
            )
        else:
    ...
    @staticmethod
    def push_to_server_whitelist(
            player_id: str, game_server: MCServerConnection, mode=1
    ) -> (int, str):  # mode=1加模式，否则为删模式
    ...
    @staticmethod
    def push_to_server_command(
            command: str, game_server: MCServerConnection
    ) -> (int, str):  # 用于推送指令
        return game_server.push_command(command=command)
