...
class Listener:
    def __init__(
            self, wx_user_name, responses: dict, url: str, game_server: MCServerConnection
    ):
        ...
        def wx():
            if request.method == "POST":
                ...
                type_of_message = Listener.message_type(
                    data
                ...
                if type_of_message == -1:  # 非POST消息
                    ...
                    raw_command = (
                        self.xml_data.getElementsByTagName("Content")[0]
                        .childNodes[0]
                        .data
                    )
                    open_id = (
                        self.xml_data.getElementsByTagName("FromUserName")[0]
                        .childNodes[0]
                        .data
                    )
                    flag, message = CommandLine.command_node(
                        command=raw_command,
                        open_id=open_id,
                        responses=__responses,
                        game_sever=game_server,
                    )
                    if flag != 0:
                        return "无回复"
                    else:
                        return WxConnection.message_encode(
                            openid=open_id, weid=self.WxUserName, message=message
                        )
                    ...
                    if player_id is not None:  # 取消订阅的人是玩家
                        try:
                            Listener.remove_whitelist(
                                player_id, open_id, game_server=game_server
                            )
                            ...
                            cur = conn.cursor()
                            cur.execute(
                                "UPDATE players SET ISSUB=? WHERE OPENID=?",
                                (1, open_id),
                            )
                            conn.commit()
                            cur.close()
    ...
    def message_type(data):
        ...
        try:
            ...
            message_type = (
                xml_data.getElementsByTagName("MsgType")[0].childNodes[0].data
            )
        except:
            return -1
        else:
            if message_type == "text":
                ...
                if (
                        xml_data.getElementsByTagName("Event")[0].childNodes[0].data
                        == "unsubscribe"
                ):
                    return 1
                else:
